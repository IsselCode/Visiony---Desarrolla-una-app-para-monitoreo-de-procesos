...
class GeneralStatisticsState extends ChangeNotifier {
  ...
  Future<void> getAllData() async {
    ...
    machinesQuantity = await getMachinesQuantity();
    linesQuantity = await getLinesQuantity();

    notifyListeners();
  }

  Future<int> getMachinesQuantity() async {
    CollectionReference colRef = firestore.collection("maquinas");
    AggregateQuerySnapshot querySnap = await colRef.count().get();
    return querySnap.count ?? 0;
  }

  Future<int> getLinesQuantity() async {
    CollectionReference colRef = firestore.collection("lineas");
    AggregateQuerySnapshot querySnap = await colRef.count().get();
    return querySnap.count ?? 0;
  }
  ...
  Future<List<List<int>>> getPartBySixHourIntervals(DateTime currentDate) async {
    ...
    print(currentTimeWithoutMinutes);
